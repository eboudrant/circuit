...
class SubCircuitSymbolProcessorTest {
  ...
  private val multibindingAnnotations =
    kotlin(
      "MultibindingAnnotations.kt",
      """
  ...
  fun presenterFactory_hilt() {
    ...
    // The factory itself is identical to the Anvil output minus the @ContributesMultibinding
    // annotation, so it can be asserted exactly.
    assertThat(
        readGeneratedFile(generatedSourcesDir, "test/TestPresenter_Factory_SubPresenterFactory.kt")
      )
      .isEqualTo(
        """
        // Generated by SubCircuitSymbolProcessor
    ...
    // The generated Hilt module's @Binds signature is long enough to be reformatted by KotlinPoet's
    // line wrapping, so assert the structurally-significant tokens rather than exact text.
    val module =
      readGeneratedFile(
        generatedSourcesDir,
        "test/TestPresenter_Factory_SubPresenterFactoryModule.kt",
      )
    assertThat(module).contains("import dagger.Module")
    assertThat(module).contains("import dagger.Binds")
    assertThat(module).contains("import dagger.hilt.InstallIn")
    assertThat(module).contains("import dagger.multibindings.IntoSet")
    assertThat(module).contains("import com.slack.circuit.subcircuit.SubPresenterFactory")
    assertThat(module).contains("@Module")
    assertThat(module).contains("@InstallIn(AppScope::class)")
    assertThat(module)
      .contains("public abstract class TestPresenter_Factory_SubPresenterFactoryModule")
    assertThat(module).contains("@Binds")
    assertThat(module).contains("@IntoSet")
    assertThat(module).contains("fun bindTestPresenter_Factory_SubPresenterFactory")
  }
  ...
  fun uiFactory_hilt() {
    ...
    assertThat(readGeneratedFile(generatedSourcesDir, "test/TestUi_SubUiFactory.kt"))
      .isEqualTo(
        """
        // Generated by SubCircuitSymbolProcessor
        package test
    ...
    val module = readGeneratedFile(generatedSourcesDir, "test/TestUi_SubUiFactoryModule.kt")
    assertThat(module).contains("import dagger.Module")
    assertThat(module).contains("import dagger.Binds")
    assertThat(module).contains("import dagger.hilt.InstallIn")
    assertThat(module).contains("import dagger.multibindings.IntoSet")
    assertThat(module).contains("import com.slack.circuit.subcircuit.SubUiFactory")
    assertThat(module).contains("@Module")
    assertThat(module).contains("@InstallIn(AppScope::class)")
    assertThat(module).contains("public abstract class TestUi_SubUiFactoryModule")
    assertThat(module).contains("@Binds")
    assertThat(module).contains("@IntoSet")
    assertThat(module).contains("fun bindTestUi_SubUiFactory")
  }
  ...
  private fun assertGeneratedFile(
    ...
    val generatedSourcesDir = compileAndAssertOk(CodegenMode.ANVIL, sourceFile)
    assertThat(readGeneratedFile(generatedSourcesDir, generatedFilePath)).isEqualTo(expectedContent)
  }

  private fun readGeneratedFile(generatedSourcesDir: File, generatedFilePath: String): String {
    ...
    return generatedFile.readText().trim()
  }

  private fun compileAndAssertOk(mode: CodegenMode, sourceFile: SourceFile): File {
    val compilation = prepareCompilation(mode, sourceFile)
  ...
  private fun assertProcessingError(sourceFile: SourceFile, body: (messages: String) -> Unit) {
    val compilation = prepareCompilation(CodegenMode.ANVIL, sourceFile)
    val result = compilation.compile()
  ...
  private fun prepareCompilation(
    ...
    KotlinCompilation().apply {
      ...
      configureKsp {
        kspProcessorOptions += "subcircuit.codegen.mode" to mode.name
